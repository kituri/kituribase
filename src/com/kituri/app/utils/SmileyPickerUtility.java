...
public class SmileyPickerUtility {
    public static void hideSoftInput(View paramEditText) {
        ((InputMethodManager) KituriApplication.getInstance().getSystemService(Context.INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(paramEditText.getWindowToken(), 0);
    }
    ...
    public static void showKeyBoard(final View paramEditText) {
        ...
        paramEditText.post(new Runnable() {
            @Override
            public void run() {
                ((InputMethodManager) KituriApplication.getInstance().getSystemService(Context.INPUT_METHOD_SERVICE)).showSoftInput(paramEditText, 0);
            }
        });
    }
